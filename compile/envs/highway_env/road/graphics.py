...
class RoadGraphics(object):
    ...
    @staticmethod
    def display_custom_trajectory(surface: WorldSurface, offscreen: bool = False, traj_list=[]) -> None:
        RoadObjectGraphics.display_custom_trajectory_object(surface=surface, offscreen=offscreen, traj_list=traj_list)
...
class RoadObjectGraphics:
    ...
    YELLOW = (235, 189, 52)
    BLUE = (100, 200, 255)
    CUST_TRAJECTORY_COLOR = (230, 230, 250, 0.9)
    RED = (255, 100, 100)
    GREEN = (50, 200, 0)
    BLACK = (60, 60, 60)
    ...
    @classmethod
    def display_custom_trajectory_object(cls, surface: WorldSurface, offscreen: bool = False, traj_list=[]) -> None:
        #s = pygame.Surface((surface.get_height(), surface.get_width()), pygame.SRCALPHA)
        color = list(cls.CUST_TRAJECTORY_COLOR)
        for point in traj_list:
            pygame.draw.circle(surface, tuple(color), (surface.pos2pix(point[0],point[1])), 5)
            color[2] =min(255,color[2]+5)
    ...
    def get_color(cls, object_: 'RoadObject', transparent: bool = False):
        ...
        if isinstance(object_, Obstacle):
            ...
            if object_.hit:
                # indicates success
                color = cls.BLUE #Megha: Originally green, change to not confuse users!
            else:
                color = cls.BLUE
